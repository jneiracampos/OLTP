...
public class SQLEstante {
	...
	public long adicionarEstante (PersistenceManager pm, long idEstante, int capacidadTotal, int nivelAbastecimiento, int categoriaProducto, long id_sucursal, int capacidadDisponible)
	{
		Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaEstante () + " values (?, ?, ?, ?, ?, ?)");
		q.setParameters(idEstante, capacidadTotal, nivelAbastecimiento, categoriaProducto, id_sucursal, capacidadDisponible);
	...
	public Estante darEstantePorIdSucursalIdCategoriaProducto(PersistenceManager pm, long idSucursal, long idCategoriaProducto) {
		Query q = pm.newQuery(SQL, "SELECT * FROM " + pp.darTablaEstante() + " WHERE id_sucursal = ? AND id_categoria_producto = ?");
		q.setResultClass(Estante.class);
		q.setParameters(idSucursal, idCategoriaProducto);
    ...
    {
        Query q = pm.newQuery(SQL, "UPDATE " + pp.darTablaEstante() + " SET capacidad_disponible = ? WHERE id = ?");
